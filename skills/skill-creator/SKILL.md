...
- 用户已经在本仓库工作（cwd 包含 `.claude-plugin/marketplace.json`）
...
1. **确认 cwd**：必须包含 `.claude-plugin/marketplace.json` 与 `.claude-plugin/plugin.json`。否则报错并退出："当前目录不是 SharkerSkills plugin 根目录，请 `cd` 到正确仓库后重试。"
2. **读取当前版本**：`jq -r .version .claude-plugin/plugin.json` → 记为 `<current_version>`。
