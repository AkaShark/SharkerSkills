...
> Claude 加载本 skill 时会得到一个「Base directory」路径（形如 `~/.claude/plugins/cache/sharker-skills/sharker-tool/<version>/skills/picgo-upload`）。**下文用 `$SKILL_DIR` 指代该路径**；实际执行时把它替换成完整路径，bash 中可写成 `SKILL_DIR=<base_dir>` 后续复用。
